...
public class OctoBomber : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        if (!spawning)
        {
            ...
            {
                animator.SetInteger("Progress", 50);
            }
        }
        else
        {
    ...
    private void Shoot(Vector2 position) // Crea un proyectil nuevo
    {
        ...
        if(VoicePitch == 1f)
            newPos = new Vector2(transform.position.x + dir * 2f, transform.position.y - 1f);
        if(VoicePitch == 0.5f)
            newPos = new Vector2(transform.position.x + dir * 0.3f, transform.position.y + 2f);
    ...
    private Vector2 Trajectory(Vector2 TargetPosition, float Overshoot){
        ...
        float TargetDistanceX = TargetPosition.x - transform.position.x + Random.Range(-XError, XError);
        float TargetDistanceY = TargetPosition.y - transform.position.y;

        float XCorrectApplicator = XCorrect * (TargetDistanceX / Mathf.Abs(TargetDistanceX));
        TargetDistanceX = TargetDistanceX - XCorrectApplicator;
		...
		ComponentY = Mathf.Sqrt(2 * (9.81f * 3) * (MaxPointY));
		
		Time = Mathf.Sqrt(2 * (9.81f * 3) * (MaxPointY - TargetDistanceY)) + ComponentY;
		Time = Time / (9.81f * 3);
