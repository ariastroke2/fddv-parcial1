...
public class Splatling_Behav : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        tempObject.GetComponent<UI_behavior>().ChangeWeapon("Splatling", gameObject);
        ...
        if (Release && Charge > 0)
        {
            if (cooldown < 1)
            {
                ...
                else
                {
                    ...
                    obj.GetComponent<Bullet_Behaviour>().size = BulletSize * 0.8f;
                    obj.GetComponent<Bullet_Behaviour>().maxDistance = MaxRange * 0.75f;
                    obj.GetComponent<Bullet_Behaviour>().dmg = BulletDmg / 2;
                }
            }
            if (Charge < 1)
            {
